export const items = [
	{
		icon: "/icons/peticion.svg",
		title: "Petición",
		descripcion:
			"Es el derecho fundamental que tiene toda persona a presentar solicitudes respetuosas a las autoridades por motivos de interés general o particular y a obtener su pronta resolución.",
		cta: "Escribir petición",
		url: "mailto:info@fonpacifico.org?subject=Peticion",
	},
	{
		icon: "/icons/queja.svg",
		title: "Queja",
		descripcion:
			"Es la manifestación de protesta, censura, descontento o inconformidad que formula una persona en relación con una conducta que considera irregular de uno o varios servidores públicos en desarrollo de sus funciones.",
		cta: "Escribir queja",
		url: "mailto:info@fonpacifico.org?subject=Queja",
	},
	{
		icon: "/icons/reclamo.svg",
		title: "Reclamo",
		descripcion:
			"Es el derecho que tiene toda persona de exigir, reivindicar o demandar una solución, ya sea por motivo general o particular, referente a la presentación indebida de un servicio o a la falta de atención de una solicitud.",
		cta: "Escribir reclamo",
		url: "mailto:info@fonpacifico.org?subject=Reclamo",
	},
	{
		icon: "/icons/sugerencia.svg",
		title: "Sugerencia",
		descripcion:
			"Es la manifestación de una idea o propuesta para mejorar el servicio o la gestión de la entidad.",
		cta: "Escribir sugerencia",
		url: "mailto:info@fonpacifico.org?subject=Sugerencia",
	},
	{
		icon: "/icons/denuncia.svg",
		title: "Denuncia",
		descripcion:
			"Es la puesta en conocimiento ante una autoridad competente de una conducta posiblemente irregular, para que se adelante la correspondiente investigación penal, disciplinaria, fiscal, administrativa - sancionatoria o ético profesional.",
		cta: "Escribir denuncia",
		url: "mailto:info@fonpacifico.org?subject=Denuncia",
	},
];
